    ...
    <el-table v-loading="loading" :data="list" stripe>
      ...
      <el-table-column label="操作" width="150" fixed="right">
        <template #default="{ row }">
          ...
          <el-button v-hasPermi="['restaurant:notice:delete']" link type="danger" @click="onDelete(row)">
            删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <Dialog v-model="dialogVisible" :title="dialogTitle" width="560px">
      <el-form label-width="90px">
        ...
        <el-form-item label="标题" required>
          <el-input v-model="form.title" placeholder="如 国庆营业时间调整（≤100 字）" />
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="saving" @click="onSubmit">发 布</el-button>
      </template>
    </Dialog>
  </ContentWrap>
</template>
...
const getList = async () => {
  loading.value = true
  try {
    const data = await RestaurantApi.getNoticeList(queryStoreId.value)
    list.value = data || []
  } finally {
    loading.value = false
  }
...
const openForm = (row?: any) => {
  dialogTitle.value = row ? '编辑公告' : '发布公告'
  Object.assign(form, {
    id: row?.id,
...
const onSubmit = async () => {
  ...
  try {
    if (form.id) {
      await RestaurantApi.updateNotice({ ...form })
    } else {
      await RestaurantApi.createNotice({ ...form })
    }
...
const onDelete = async (row: any) => {
  try {
    await message.delConfirm(`确认删除公告「${row.title}」？`)
  } catch {
    return
  }
  await RestaurantApi.deleteNotice(row.id)
